import{a as p}from"./vendor-a61d8330.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const c of r)if(c.type==="childList")for(const i of c.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function o(r){const c={};return r.integrity&&(c.integrity=r.integrity),r.referrerpolicy&&(c.referrerPolicy=r.referrerpolicy),r.crossorigin==="use-credentials"?c.credentials="include":r.crossorigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function s(r){if(r.ep)return;r.ep=!0;const c=o(r);fetch(r.href,c)}})();const g="https://food-boutique.b.goit.study/api";async function E(){try{return await p.get(`${g}/products/categories`)}catch(t){console.log(t.message)}}async function f(t){try{return(await p.get(`${g}/products/${t}`)).data}catch(e){console.log(e.message)}}async function A(){try{return await p.get(`${g}/products/popular?limit=5`)}catch(t){console.log(t.message)}}async function I(){try{return await p.get(`${g}/products/discount`)}catch(t){console.log(t.message)}}const a={filterCatList:document.querySelector(".filters-categories"),form:document.getElementById("fiters-form"),filtersInput:document.querySelector(".filters-input"),filtersCategories:document.querySelector(".filters-categories")};a.form.addEventListener("submit",B);x();async function x(){try{const e=(await E()).data;for(let o=0;o<e.length;o++){let s=v(e[o]);a.filterCatList.insertAdjacentHTML("beforeend",`<option  class="filters-categories-option" value="${s}">${s}</option>`)}}catch{console.log("err")}}const l={keyword:null,category:null,page:1,limit:6};async function B(t){const e=a.filtersInput.value;console.log(e),l.keyword=e,e===""&&(l.keyword=null);const o=a.filtersCategories.value;l.category=o,o===""&&(l.category=null),localStorage.setItem("filters",JSON.stringify(l))}M();function M(){const t=JSON.parse(localStorage.getItem("filters"));a.filtersInput.value=t.keyword,a.filtersCategories.value=t.category}function v(t){return t.replace(/_/g," ")}async function q(t,e,o=1,s=6){const r=`https://food-boutique.b.goit.study/api/products?keyword=${t}&category=${e}&page=${o}&limit=${s}`;try{return(await(await fetch(r)).json()).results}catch(c){return console.error("Error fetching products:",c),[]}}function O(){const t=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;return t>=1440?9:t>=768?8:6}function T(t){let e=v(t.category);return`
  ...
  `}const z=localStorage.getItem("filters"),u=JSON.parse(z);async function y(t=1){let e=u.keyword;u.keyword===null&&(e="");let o=u.category;u.category===null&&(o="");const s=O();try{const c=await q(e,o,t,s),i=document.getElementById("productList");i.innerHTML="",c.forEach(C=>{i.innerHTML+=T(C)});const k=Math.ceil(c.length/s);renderPagination(k,t)}catch(r){console.error("Помилка:",r)}}document.addEventListener("DOMContentLoaded",()=>{y()});window.changePage=function(t){y(t),console.log(t)};window.addEventListener("resize",y);var S=document.createElement("script");S.src="/js/pagination.js";document.head.appendChild(S);function H(t,e){return t.sort(()=>.5-Math.random()).slice(0,e)}function N(t){let e=v(t.category);return`
  ...
  `}A().then(t=>t.data).then(t=>{const e=H(t,5),o=document.getElementById("popularContainer");e.forEach(s=>{o.innerHTML+=N(s)})}).catch(t=>{console.error(t.message)});function _(t,e){return t.sort(()=>.5-Math.random()).slice(0,e)}function F(t){return`
...
//# sourceMappingURL=cart-3b9cc0b0.js.map
